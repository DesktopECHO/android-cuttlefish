...
ln -s /usr/lib/cuttlefish-common/bin/graphics_detector %{buildroot}/usr/lib/cuttlefish-common/bin/aarch64-linux-gnu/gfxstream_graphics_detector
ln -s /usr/lib/cuttlefish-common/bin/libvk_swiftshader.so %{buildroot}/usr/lib/cuttlefish-common/bin/aarch64-linux-gnu/libvk_swiftshader.so
ln -s /usr/lib/cuttlefish-common/bin/graphics_detector %{buildroot}/usr/lib/cuttlefish-common/bin/x86_64-linux-gnu/gfxstream_graphics_detector
ln -s /usr/lib/cuttlefish-common/bin/libvk_lavapipe.so %{buildroot}/usr/lib/cuttlefish-common/lib64/vulkan.lvp.so
ln -s /usr/lib/cuttlefish-common/bin/libvk_swiftshader.so %{buildroot}/usr/lib/cuttlefish-common/lib64/vulkan.pastel.so
...
%post -n cuttlefish-integration
systemctl daemon-reload >/dev/null 2>&1 || :
systemctl enable --now cuttlefish-defaults.service >/dev/null 2>&1 || :
...
%preun -n cuttlefish-integration
if [ $1 -eq 0 ]; then
...
%postun -n cuttlefish-integration
systemctl daemon-reload >/dev/null 2>&1 || :
...
%changelog
* Sat Mar 28 2026 GitHub Copilot <copilot@openai.invalid> - 1.50.0-1
